...
public interface StatisticRepository extends JpaRepository<StatisticEntity, Long> {
    ...
    // Integer profileId);
    @Query("SELECT new com.example.ghtkprofilelink.model.dto.StatisticDto(s.id,s.clickCount,s.profileId,s.date,p.avatarLink,p.fullname,p.shortBio) FROM StatisticEntity s, ProfileEntity p where p.id=s.profileId and month(date) = :month and year(date) = :year ORDER BY s.clickCount DESC")
